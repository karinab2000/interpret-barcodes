...
task RegressionOutput {
    ...
    size([barcodes_file, barcode_pos, pickle_file], "GB")
    )
    ...
    command <<<
        ...
        data_df = DataFrame(data_df)
        meta_dict = defaultdict(list)
        region_codes = {}
        for i, region in enumerate(np.unique(data_df['Region'])):
            region_codes[region] = i+1
            
        for region, year in zip(data_df['Region'], data_df['Year']):
            if year == 2019:
                if region not in ['SLP']:
                    meta_dict['is_cachement'].append(True) # is cachement -- do we use region-wide estimates or local estimates, Julie's document is local estimate
                else:
                    meta_dict['is_cachement'].append(False)
            else:
                meta_dict['is_cachement'].append(False)
            meta_dict['region_cat'].append(region_codes[region])
        for key in meta_dict:
            data_df[key] = meta_dict[key]

        with open('~{pickle_file}', "rb") as file:  # "rb" stands for read-binary
            model = dill.load(file)

        x_of_interest = ["Year", "poly_fract", "monoclonality", "RH", "cotx", "mccoil_coi", "is_cachement"]
        subset_df = data_df[x_of_interest].copy()
        subset_df = pd.DataFrame(subset_df)
        subset_df['Incidence'] = 0
        subset_df['region'] = data_df['Region']
        ...
        for model_idx, label in model_mapping.items():
            ...
            for number in range(len(np.asarray(list(model_data.values())).T)):
                data = np.asarray(list(model_data.values())).T[number]
                mean = np.mean(data)
                standard_error = stats.sem(data)
                confidence_interval = stats.t.interval(0.95, len(data)-1, loc=mean, scale=standard_error)
            ...
            subset_df[f'Incidence_{label}'] = mean_values
            subset_df[f'lower_ci_{label}'] = lower_ci
            subset_df[f'upper_ci_{label}'] = upper_ci

        subset_df = subset_df.drop(columns=['Incidence'])
        subset_df = subset_df.rename(columns={'monoclonality': 'monogenomic__clone_fraction'})

        # Save results to CSV
        subset_df.to_csv("regression_output.csv", index=False)
    ...
    runtime {
        ...
        preemptible: 0
        maxRetries: 0
        docker: "karinab2000/interpret-barcodes-python:v4"  
    }
}
